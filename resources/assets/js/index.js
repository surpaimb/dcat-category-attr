(function () {
    function SKU(wrap) {
        this.wrap = $(wrap);
        this.attrs = {};
        this.categoryAttributes = JSON.parse($('.category_attributes').val());
    ...
    SKU.prototype.init = function () {
        ...
        _this.wrap.find('.sku_attr_key_val').on('change', '.attribute_selector', function () {
            if ($(this).val() != 'input') {
                let categoryAttributesArray = _this.categoryAttributes;
                _this.attrIndex = $(this).find("option:selected").data('idx');
                _this.skuAttr = categoryAttributesArray[_this.attrIndex];
            }
        ...
        if (old_val) {
            ...
            attr_keys.forEach(function (attr_key, index) {
                ...
                let scopeAttrType = '';
                let sku = _this.categoryAttributes.filter((res) => {
    ...
    SKU.prototype.SKUForm = function (default_sku, params) {
        ...
        if (attr_names.length === 0) {
            ...
            // 求笛卡尔积
            let cartesianCategoryOf = (function () {
            ...
            // 根据计算的笛卡尔积渲染tbody
            let tbody_html = '';
            cartesianCategoryOf.forEach(function (sku_item) {
                tbody_html += '<tr>';
    ...
    SKU.prototype.getHtml = function (innerHtml = '') {
        let _this = this;
        let categoryAttributesArray = this.categoryAttributes;
        ...
        categoryAttributesArray.forEach(function (v, i) {
    ...
    window.SurpaimbCategoryAttribute = SKU;
})();
